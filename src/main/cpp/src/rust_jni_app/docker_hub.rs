...
impl DockerHub{
    ...
    pub fn new(container_name:String,container_version:String,cache_dir:String,arch: Option<String>) -> anyhow::Result<Self>{

        let p = Path::new(cache_dir.as_str());
        if !p.exists(){
            std::fs::create_dir_all(p)?;            
        }
        if !p.is_dir(){
            bail!("cache_dir path should be directory")
        }
    ...
    fn sha512_for_string(&self,s:&str)->String{
//        use digest;
//        let hash= hmac_sha512::Hash::hash(s);
        let hash= Sha256::digest(s);
        hash.iter().map(|x| format!("{:02x}", x)).collect::<String>()   
//        format!("{:02x?}",hash).to_string()
    }

    fn cache_filename_for_url(&self,url:&str)->String{
        let hash=self.sha512_for_string(url);
...
//        let cache_path=Path::new(cache_filename.as_str());
//        if cache_path.exists(){
//             we're have cache file
//        }
        ...
        .await?;
        //write file to cache
        tokio::fs::write(cache_filename, resp.clone()).await?;
        //return
        Ok(resp)
    }
...
//    #[test]
    ...
    async fn it_works() {
        ...
        let mut app = DockerHub::new( "zigbee2mqtt/zigbee2mqtt-armv7".to_string(),"latest".to_string(), "./".to_string(), Some("arm/v7".to_string())).unwrap();
//        let mut app = DockerHub::new( "library/node".to_string(),"slim".to_string(), "./".to_string(), Some("arm/v7".to_string())).unwrap();
//        let mut app = DockerHub::new( "library/ubuntu".to_string(),"latest".to_string(),Some("arm/v7".to_string()));
        let (layers,_container_conf_opt)=app.get_layers_urls(None).await.unwrap();

        for (layer_url,layer_format)  in layers{
            println!("unarchiving blob {} with type {}",layer_url,layer_format);
            let bin=app.download_blob(layer_url).await.unwrap();
